...
import meadRecipeRouter from './routers/mead-recipe-router.js';
...
const meadRecipeMount = express.Router();
...
function refreshRouters(config: AppConfig) {
  ...
  vtuberMount.stack = [];
  orgMount.stack = [];
  meadRecipeMount.stack = [];

  // 3) Add new routes
  vtuberMount.use(vtuberRouter(config));
  orgMount.use(orgRouter(config));
  meadRecipeMount.use(meadRecipeRouter(config));
}
